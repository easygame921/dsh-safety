...
> 状态：**M0-M2 已完成（2026-08-16）**，M3/M4 待做。
...
| 阶段 | 内容 | 验收 | 状态 |
|---|---|---|---|
| M0 规则库 v1 | 规则表（T01–T06）落地 + 单元测试夹具（良性/恶意插件样例） | 恶意夹具全命中、良性零误报（目标） | ✅ 完成（9/9 测试通过） |
| M1 扫描引擎 | 文件发现 + 规则执行 + 证据收集 + 报告生成 | CLI 可对本地目录出报告 | ✅ 完成 |
| M2 MCP 服务器 | 四个 MCP 工具 + npm/GitHub 下载解析 | DSH 会话可调用并拿到结构化报告 | ✅ 完成（smoke PASS；npm/GitHub/本地三种来源实测） |
| M3 加固 | 混淆检测增强、P1 规则、缓存与限流 | P1 规则上线 | ✅ 完成（T07-T11 + 白名单机制，17/17 测试） |
| M4 开源 | README/文档/贡献指南、CI（lint+test）、发布 npm + GitHub Release | 可被社区安装使用 | ⏳ 待做（暂不开源，按用户决定；CI 已配置） |
